...
def adminlm(request) :
    ...
    username = request.session.get('username')
    context1 = list_output(username, details1)
    return render(request, "Admin_LM.html", context1)
...
def adminloanapproval(request) :
    ...
    if int(count[0]) >0 :
        ...
        context = {'username': request.session.get('username'), 'length_list': length_list, 'requestid': requestid, 'firstname': custfirstname, 'lastname': custlastname, 'dob': custdob, 'age': custage, 'agentrequested': agentrequested, 'requestedamount': requestedamount, 'requestedtenure': requestedtenure, 'alottedroi': alottedroi, 'monthlyinterest': monthlyinterest, 'count': int(count[0])}
        return render(request, "Admin_Loan_Approval.html", context)
    context = {'username': request.session.get('username'), 'count': int(count[0])}
    return render(request, "Admin_Loan_Approval.html", context)
...
def agentlm(request) :
    cursor = connections['default'].cursor()
    cursor.execute("SELECT * from redcarpetuplmsdb where agentserviced = '{}'".format(request.session.get('username')))
    details = cursor.fetchall()
    username = request.session.get('username')
    context = list_output(username, details)
...
def agentloanrequest(request) :
    ...
    context = {'form': form, 'username': request.session.get('username')}
...
def customerlm(request) :
    cursor = connections['default'].cursor()
    cursor.execute("SELECT * from redcarpetuplmsdb where custfirstname = '{}'".format(request.session.get('username')))
    details = cursor.fetchall()
    username = request.session.get('username')
    context = list_output(username, details)
...
def list_output(username, details):
    ...
    context = {'username': username, 'length_list': length_list, 'loan_id': loanid,
